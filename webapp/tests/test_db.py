...
async def test_fresh_v14_schema_uses_v13_helper_and_exact_tombstone_ddl(
    ...
    assert db.TARGET_USER_VERSION == 28
    assert calls == 1
...
@pytest.mark.asyncio
async def test_v23_coach_agent_runs_have_explicit_idempotent_system_trigger_contract():
    conn = await db.get_conn()

    columns = {
        row[1]: row
        for row in await (await conn.execute(
            "PRAGMA table_info(coach_agent_runs)"
        )).fetchall()
    }
    assert columns["initiator"][4] == "'user'"
    assert "trigger_ref" in columns

    index = await (
        await conn.execute(
            "SELECT sql FROM sqlite_master WHERE type='index' "
            "AND name='idx_coach_agent_runs_owner_system_trigger'"
        )
    ).fetchone()
    assert index is not None
    assert "WHERE initiator='system' AND trigger_ref IS NOT NULL" in index[0]
