...
def test_d1_full_backfill_of_frozen_cleaned_tree(
    ...
    assert expected == 93, "frozen cleaned/ tree changed; re-pin DoD expectations"
...
def test_d6_label_passthrough_is_field_exact_and_never_touches_overrides(
    ...
    for key, entry in _ledger_runs().items():
        ...
        if not scenario_path.is_file():
            # 0901 起导入器对无 scenario.json 的轮也会生成行为提案
            # （label/score/margin/coverage/score_parts）；旧冻结 meta 保持裸
            # pending。两者均合法——本用例核心不变量是「不触碰 override」。
            assert proposal == {"source": "pending"} or (
                "label" in proposal and "score" in proposal
            )
            pending += 1
            continue
    ...
    assert passthrough + pending == 93
    assert pending == 31  # 030352 9 + verify0831 3 + 0901 四个会话（2021/2132/2156/2359）共 19 轮无 scenario.json（每次录制会话都会增长，长期应改为固定夹具）
...
def test_d5_unpaired_rounds_without_any_runs_stay_healthy(
    ...
    assert summary["imported"] == 93 and summary["failed"] == 0
