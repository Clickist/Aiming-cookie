...
def test_d1_full_backfill_of_frozen_cleaned_tree(
    ...
    assert expected == 82, "frozen cleaned/ tree changed; re-pin DoD expectations"
...
def test_d6_label_passthrough_is_field_exact_and_never_touches_overrides(
    ...
    assert passthrough + pending == 82
    assert pending == 20  # 030352 主目录 9 轮 + verify0831（0831_192538）3 轮 + 0901 三个新会话（2021/2132/2156）共 8 轮无 scenario.json
...
def test_d5_unpaired_rounds_without_any_runs_stay_healthy(
    ...
    assert summary["imported"] == 82 and summary["failed"] == 0
