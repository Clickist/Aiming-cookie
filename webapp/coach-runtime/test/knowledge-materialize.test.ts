...
test("the v11 registry materializes all 51 entries", () => {
  materializeKnowledgeDir();
  const index = JSON.parse(readFileSync(join(knowledgeDir, "index.json"), "utf-8")) as {
    registry_version: string;
    entries: Array<{ entry_file: string }>;
  };
  assert.equal(index.registry_version, "2026-09-10.v11");
  assert.equal(index.entries.length, 51);
  assert.equal(readdirSync(join(knowledgeDir, "entries")).length, 51);
});
