...
- **CV 家族泛化施工完成（点点决策「特定场景价值不大」，主会话实施，本地 commit 未推送）**：移动靶速度自适应跟踪（EMA 速度 + 预测门控，静止场景行为不变；轨迹新增有界 path 序列）；`kovaak_tracker/generic_aim_family_analysis.py` 三家族层——dynamic（击中判定点击时刻路径位置 + 真实样本优先 + 100ms 紧回看 + 不信任 degraded 中心兜底 + 靶速度指标）、switching（kill 边界转火 episode：转火时间/下一靶配对 + generic_switch_episode 事件）、tracking（最近靶误差序列：误差中位/p90、in-target 比、丢失计数、覆盖 Gate）；worker 分派泛化到全部四家族 baseline（`{family}.generic_visual.v1` 版本升级 + Gate 失败回退不变）；证据扩展 `generic-aim-families@1` Py/TS 双侧注册。**实测**：54005（1wall5targets_pasu，dynamic）假设选择暗簇/球与人工校准 pasu 档殊途同归、移动轨中位存活 684ms（速度门控保持身份）、击中 108/133 vs Stats 真值 97 中（约 81% 精度，余量是视觉框 vs 判定框的边缘模糊，如实保留）；静态 54006 重构后零回退（125/114 不变）。**素材边界（如实记录）**：tracking 无真实视频（54002 缺 MP4）、switching 家族无任何真实 run——这两层只有合成测试覆盖，待点点打一局再实测。验证：Python 1187/5、TS 证据面绿、新增 4 家族层测试。
