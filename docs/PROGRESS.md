...
- **Shutdown ordering race 已修复并纳入回归，但继续保留发布观察。** Tauri 在 `CloseRequested` 即发起幂等 shutdown，Python 先停止 capture-control 消费方再关闭原生 endpoint/服务；既有真实 `Alt+F4` targeted regression 中 `capture_control_unavailable`、`capture_export_failed`、ASGI traceback 均为 `0`。2026-07-26 一次较长 UI smoke 关闭出现两条 `capture_export_failed`，随后的独立最小启动/关闭复现为关闭前后均 `0`，所有进程和端口干净退出；当前不足以重开 blocker，但干净关闭必须继续作为发布回归项。
...
- 当前发布状态仍为 **No-Go**。
- Versioned Coach Knowledge Registry v1 已完成并保持 legacy/historical contract：43 条 active entry 覆盖早期 Flicking、Tracking、身体/张力候选解释、settings、practice 与 verification，Python/TypeScript 共用确定性检索，历史 trace 只保存版本化引用。Task 10R 审计确认 v1 的通用 prose shape 不足以机器强制跨 family 的 quality/direction/alternatives/cue/dose/matched/near-transfer 闭环；不能把“v1 已完成”解释成完整 Coach 知识已完成。
