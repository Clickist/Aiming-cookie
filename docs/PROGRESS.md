...
- **家族泛化实测补全（点点打图 54010-54012，主会话实测后修正）**：假设门槛为单靶短存活场景放宽（采样 25→40 帧、960 宽、帧内靶数下限取消——patCircleSwitch 此前被「中位靶数≥1」错杀）；switching 层 hold-fire 支持（轨迹 95% 按住时点击判定停用 + 转火改 kill→kill + 配对前窗 +400ms——小靶快速生成的死亡登记滞后）+ 击中/漏判几何全面改「点击时刻路径位置、真实样本优先」。**实测结果**：54011（patCircleSwitch small NR，switching）配对 39/53=73.6% Gate 过、52 个 kill→kill 转火 episode p50=1115ms；54010（Smoothsphere Viscose Easier）追踪层手动实测：覆盖 99.9%、误差中位 0.67°/p90 1.91°、在靶率 86%、零丢失。**已知偏差**：54010 被场景分类器判为 static_clicking（产品路径会走静态层而非追踪层）——如需按追踪分析可在 app 里纠正分类（scenario memory）；追踪层产品路径仍无真实验证 run。验证：Python 1187/5 全绿。
