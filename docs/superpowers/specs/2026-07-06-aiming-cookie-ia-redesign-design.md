...
### 2.2 商业:D 邀请制 → B freemium(分阶段)
- **D 阶段(邀请制内测,pre-launch)**:不开放公开注册,靠邀请码;login 页是 "Early Access" 叙事,无定价、无 credits;成本靠手动控量;筛高质量反馈用户。
...
| 场景 | 流程 |
|---|---|
| **注册**(D 阶段) | email + 邀请码 → OTP 验证邮箱 → 设密码 → 进 |
| **注册**(B 阶段) | email → OTP 验证邮箱 → 设密码 → 进(邀请码退役) |
| **常规登录** | email + 密码 |
...
### 5.1 首次
```
web 官网 → 下载安装 → 启动应用 → login(email + 邀请码 + OTP + 设密码)
