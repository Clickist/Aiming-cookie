...
```text
选择训练来源 → 检查 Evidence → 确认分析模式 → 开始后台分析
```

页面必须同时支持：

- 选择 Desktop 已发现的 KovaaK Run；
- 检查 Stats、Performance、Raw Input、MP4 与时间对齐；
- 为 Run 添加可选 MP4 视觉证据；
- 在没有可用 Run 时使用 MP4 + Stats 的 video-fallback；
- 显示本次使用的 Profile 摘要，并把复杂修改引导到 Settings；
- 解释当前模式为何可用或不可用、能生成什么、不能生成什么；
- 只在必要证据、权限和一致性检查通过后允许开始。
...
前端不得仅根据文件名、平台或局部字段猜测 input mode，不得允许视觉选择覆盖真实 evidence 条件。分析模式和可用性来自稳定产品合同；界面负责解释，不负责发明。
...
- 已有关联 Analysis 及其状态；
- 当前可执行的下一步，例如开始分析、添加 MP4、修复来源或查看已有结果。
...
| 能力 | Desktop | Web |
|---|---|---|
| KovaaK Run 自动发现 | 支持本地发现与回访 | 不支持本地自动发现；使用浏览器可提供的手动来源 |
...
| MP4 | 依实际来源区分用户源与受管副本 | 删除前明确影响；不能仅凭“视频”类型猜测所有权 |
...
- Run/trace 的长期删除、tombstone 和 retention 若没有上游 lifecycle 授权，正式 v1 不提供猜测式删除控制；
...
15. 删除 terminal Analysis 不会删除 Run-owned trace、用户源文件或 Coach 历史，失效引用正确显示；
