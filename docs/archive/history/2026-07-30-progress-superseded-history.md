# Aiming Cookie Progress superseded history

> Moved from [`../../PROGRESS.md`](../../PROGRESS.md) on 2026-07-30. This file preserves the prior dated status, blockers, and validation records superseded by the current Progress snapshot; it is historical evidence, not current implementation authority.
...
- **恢复能力按已完成实现管理。** Worker 在 claim 前与 idle 时恢复 stale jobs，重启后的 Run/Analysis/finalization reconciliation、Coach 持久对话继续、停止生成、失败轮次隔离与 TeachingSession 继续均已有实现和回归；后续保留针对真实 Provider 断流/压力的可选验证，但不再把“worker 重启恢复”或“长对话恢复”写成未实现功能。
- **当前硬件与认证范围收窄。** v1 只对已有 NVIDIA 实机证据的路径作支持声明；AMD/Intel 标记为未验证且不承诺支持，不再作为当前发布 No-Go 条件。OAuth/device-code 延后，首发使用已经接通并验证的 Provider 连接方式，不让未来认证方式阻塞当前闭环。
- **当前 No-Go 仍由可执行 Gate 决定。** 真实 Tauri product-path、高 polling-rate、Tracking `<=130s` 目标和 installer/签名/updater/下载验证仍未闭合；Static、Dynamic、Tracking、Switching 当前各有一个 active exact scenario，Roadmap 不再保留“只有 Tracking active”的旧描述。
- **用户侧成绩命名保持中性。** 产品只称“KovaaK 成绩”或“训练项目成绩”，不突出外部作者、课程代号或阶段体系；内部 catalog/source refs 继续用于版本、配对与来源追溯。
- **2026-07-30 已授权连接账号与 Coach 临时查询。** 新 Task 将把本人已连接账号保存为 local owner setting，供后续手动刷新；Coach 聊天中临时给出的其它 Profile 仅在当前 turn 查询，URL/ID 不写入历史、消息、trace、audit 或 Provider 请求，成绩不进入用户的 Benchmark history 或 command audit。此前“所有外部身份只用于当次请求”的边界已被这个显式产品决定取代；OpenDesign 的 Settings/onboarding 模块需改为本地已连接账号而非一次性输入。
- **2026-07-30 后端与 Coach 接线已完成。** 本地已连接账号可保存、移除和手动刷新；刷新复用已有成绩 store，并将去身份成绩摘要交给当前 Coach agent-run 上下文。聊天里的他人 Profile 先替换为当轮内存代号，只有固定 loopback bridge 能解析；临时成绩不写 Benchmark history、消息、审计、确认或训练计划。切换已连接账号会保留旧成绩记录但标为过期，Coach 不会再把它当作当前账号成绩。
- **本轮验证。** 受影响 Python 回归 `304 passed`，Coach runtime 命令测试 `80 passed`，compileall 与 scoped `git diff --check` 通过。公开 Profile 在专用临时 SQLite 上的真实验证成功：保存账号刷新后写入完整成绩快照并进入 Coach context；临时查询后记录数不变且成绩输出不含外部身份。临时目录已送入回收站。Settings/onboarding 与最小成绩视图仍等待 OpenDesign 后的独立前端 Task。

### 2026-07-29 KovaaK score backend / Coach progression

- **训练项目与本地同步后端已接入现有 Benchmark store。** 版本化 catalog 固定 39 组两阶段配对与两个上游 benchmark ID；手动同步要求严格外部身份和本次明确同意，同时读取两个阶段、完整校验 78 个项目后一次事务写入 158 条 score/rank 记录。响应、成绩记录与 Coach context 均不保存或回显外部身份。上游会返回全部 39 个项目，完成度按最高分大于 0 的已游玩项目统计，因此完整 payload 不会被误写成 `39/39`。
- **成绩已接入原有真人 Coach 教学闭环，而不是另建一套推荐器。** `coach_turn_context.v1` 可附带去身份成绩摘要，让 Coach 先查看低档位或未完成项目；分数、排名和课程来源标签本身不能诊断 reading、张力、握法、硬件或动作原因，仍需当前 Analysis、Knowledge Registry 与 exact ScenarioProfile。已确认、可比且结果为 improved 的较低阶段复测可生成配对的更高阶段建议；它只是更难的压力测试和新基线，不代表迁移成功。更高阶段缺 exact profile 时只给名称级建议，不创建 Training Plan item 或写命令。
- **既有 Coach 改动已一起回归。** 普通问答/带练路由、候选解释、单变量练习、执行确认、复测改口、Training Plan 状态和 Provider 安全表达均保留；受影响 Python 合计 `433 passed`，完整 Coach Node `167 passed`，隐私 fixture 修正后的 benchmark focused Python `26 passed`、Node summary/tool `16 passed`，compileall 与 `git diff --check` 通过。本轮整合后最终 Python 回归为 `1210 passed, 3 skipped`，Frontend 合同测试为 `55 passed`，类型检查通过。
- **真实同步后端已通过官方当前产品路径。** 旧 `/pending/benchmark` 已被官网淘汰并返回 401；官网当前成绩页面使用匿名 `player-progress-rank-benchmark` 读取。使用点点提供的 Steam profile URL 末尾 ID，在专用临时 SQLite 上经正式 API 得到 HTTP 200、158 条原子记录、两个阶段完成度 `18/39` 与 `0/39`，且所有 `external_identity_ref` 均为空。没有 token、OAuth、EVXL 代理或网页抓取；四组最终受影响 Python 回归合计 `433 passed`。后端同步与 Coach 消费链已完成，onboarding / Settings 输入和最小成绩视图仍等待独立前端 Task。

### 2026-07-29 Analysis / Coach knowledge-boundary Task 4

- **Analysis UI 的归因语义已收口。** 具有完整 `observation_ref`、精确 Registry version 与 entry ref 的新 v2 结果显示“重点观察 / 候选解释 / 规则化练习建议”与 claim level；缺少任一 ref 的记录 fail closed 为 legacy 显示。历史 `root_causes` / `prescriptions` 保持可读但标为“历史候选说明”，不把 physical limitation、动作机制或身体状态写成已证实根因。Provider Coach 的自然表达、持久化、导航与交互均未改动。
- **前端验证通过。** Task 4 frontend unit/source `11 passed`、完整 `npm.cmd run type-check`、完整 `npm.cmd run build`、production `next start` 下 Browser smoke + accessibility `16 passed`，以及人工检查并复跑通过的 `1280x820` / `960x640` screenshot baselines。`tsconfig` 仅排除了 Git 已忽略的 `src-tauri/target` 二进制构建产物；未排除 Tauri 源码或前端产品代码。

### 2026-07-29 Analysis / Coach knowledge-boundary Task 5

- **meaningful-change 继续 fail closed。** 全仓没有任一 metric 具备 exact metric/version/conditions 的 repeatability、measurement-noise、worthwhile-change/target-band 与必要 outcome guardrail 证据，因此没有注册 improve/regress policy；History 比较仅返回严格可比的 raw delta，Coach 的非零复测继续是 `mixed_or_inconclusive`。
- **Profile 的同域绕过已修复。** 对同方向的可比 contribution，精确相等仍显示 `stable`；任意非零差异在缺少 policy 时显示 `unknown + metric_change_policy_missing`，不再呈现为 improving/deteriorating。focused history/profile/retest suites `47 passed`，最终受影响 Python 交叉套件为 `643 passed, 1 skipped`；compileall、pinned Pi Coach runtime `167 passed`、frontend lib/source `18 passed` 与 scoped `git diff --check` 均通过。
- **已记录 blocker。** Training Plan/Coach 的 `expected_metric_ref` 版本格式尚未统一：存在无版本 ref 与完整 `metric_version` ref，当前 Task 不改变该冻结合同；后续须先做 canonical metric-ref 合同迁移，再为 expected-version mismatch 增加 fail-closed 验收。

### 2026-07-27 Task 12 field blocker remediation

- **Run 479 暴露的 Data 与 video-fallback blocker 已收敛。** 新增 owner/revision-bound、path-free 的 `frontend_analysis_data.v1`，事件 marker 最多 128 个、目标相对误差序列最多 120 点；真实 Run 479 为 383 个事件、128 markers、120 samples。Data 页改用真实事件分布与目标相对误差，删除伪 Trend/Trajectory 空模块，英文技术原文只留在关闭的技术详情；指标名称、来源与 availability 使用用户语义，10 个 unavailable 指标折叠但仍可逐项检查。1280x820 / 960x640 均无横向滚动，可见内部 metric/source code 为 0。video-fallback 的 managed MP4 可用性不再依赖 EvidenceSegment 是否存在；derived artifact 损坏仍 fail-closed，HTTP 410 只降级视频局部状态。
- **Tasks failure/timestamp 一致性已修复。** stale lease 不再硬编码为 `network`，本地分析/runtime 失败使用既有 `kinematics` domain；比较时钟与真实写入时钟分离，retry attempt 保留原历史。v19 migration 只修复精确命中的未来 `stale_lease_exhausted` 异常行，不改其它历史记录。
- **Tracking CV 保持结果不变并从 45 分钟级退化恢复到约 148 秒；本轮 exact-parity 优化已按 Stop rule 收口。** production `.venv` 的 Python 3.11 / OpenCV 5.0 路径使用 `min(16, logical CPU count)`；Run 479 三次 end-to-end 为 `147.242s / 151.134s / 148.039s`，中位数 `148.039s`。三次 artifact 均与 Analysis 3 字节一致（SHA-256 `11e0d312...43ea439`，`863,126` bytes），quality `accepted`、3,600 observations、target/crosshair coverage `99.944% / 100%`。2026-07-29 profile 显示 visual `118.826s` 中原 CSRT update 为 `105.404s`（`88.7%`），decode 为 `11.949s`；完整生产协议的 CV+Tracking child 为 `127.496s`、evidence child 为 `9.289s`，未发现重复 decode/conversion 或 material import/serialization。OpenCV 单线程虽 exact-parity 但退化至 `170.665s`，已拒绝；本轮零代码改动。相对 45 分钟约快 `18.2x`，但计划目标 `<=130s` 未达到，不能写成通过；后续更快 producer 必须另过 annotation/quality Gate，不能降低帧率或改 detector/CSRT/ROI 偷换结果。
- **Desktop status 与关闭日志在产品节奏下收敛。** Rust 将 control read failure 正确分类为 `capture_unavailable`，只读 status 对 typed transport loss 最多重试三次；typed retryable 只记单行，未知异常仍保留 traceback。真实 MSVC + pinned Python Tauri 中 capture disabled / KovaaK absent 的 40 次 250ms 读取全部 available，managed media 缺失为 path-free HTTP 410；正常关窗后 `capture_export_failed`、`capture_control_unavailable/status read failed`、traceback、ASGI error 均为 0，Tauri/sidecar/Next/CDP 端口全部退出。50ms 人工压力仍曾出现 `1/120` transient unavailable，但无 traceback 或错误 export 分类；该非产品频率残余继续记录，不以堆叠重试扩大本批实现。
- **Static、Dynamic 与 Switching exact activation/field verification 均已完成当前单机行。** Static `Run 1030`（`1wall 6targets small`，hash `7378a811f430b6072d052a75896afb98`）保持已验证的 `native_flicking.v1` production path。Dynamic `Run 1032 / 1347` 均为 `pasu small reload` exact hash `a37d2ba4f3f33d59ae7018e37445a5e9`、四源完整、85.714 秒、零暂停，冻结为 calibration/untouched holdout；reviewed detector 两局分别为 `133/133` 与 `124/125`，FPR 均为 `0`，holdout 唯一 miss 为 `99px²`、低于冻结 `min_area=100`。`Run 1040` 是不同场景 `1wall5targets_pasu`，未混入 split。Switching `Run 1036 / 1038` 均为 `beanTS Larger` exact hash `3b42bdfd38a6b194737d650f3f53e8c1`、四源完整、60 秒、零暂停，冻结为 calibration/untouched holdout；最终 `Analysis 20 / 21` 使用 `Stats kill boundary -> event-local episode` 路径，分别生成 `64/64` 与 `83/83` complete chains，wall time 约 `73s / 77s`。公开事件分布、EvidenceSegment、managed MP4、Coach v3 table refs、profile contribution 与隐私 sentinel 通过；History 因 evidence coverage 未满保持 `no_comparable_baseline`。
- **最终 Dynamic `Analysis 16 / 17` 通过 partial evidence ceiling。** 两条均为 `dynamic_clicking.v1`、quality `accepted`、support `partial`，ProcessedEventTable 分别 `107/107` 与 `106/106` complete，worker wall time `111s / 137s`，artifact `3,334,224 / 3,319,603` bytes；每条有 1 个可播放 EvidenceSegment、128 个 bounded marker、Coach v3 table ref 与 8 种查询能力。normalized click error coverage 为 `9.35% / 12.26%`，贡献正确保留为 low/not-comparable 且不进入画像；History 返回 `insufficient_metric_coverage`，不生成差异百分比或伪趋势。one-shot outcome rule 未注册，acquisition、relative velocity、target-state outcome 与无可比依据的 Coach knowledge candidate 均保持 unavailable；path / CSV / Performance / protobuf / secret/token sentinel 为 `0`。
- **冻结历史正确保持，最终 Static `Analysis 11` 通过产品链。** `Analysis 6 / 7 / 8` 仍为冻结的 `scenario_outcome_only.v1`，未被改写。`Analysis 11` 从 Run 1030 生成 `native_flicking.v1`：123/123 per-flick rows complete、0 target-relative/overshoot/undershoot metric；因 0.999866 coverage 保留 `alignment_partial` 与 `support_status=partial`。`profile_contribution.v1` 写入 `static_clicking.corrective_count / peak_speed` 两条 medium/comparable dimension；`coach_diagnostic_context.v3` 为 30,640 bytes，保留 23 个指标摘要、完整 table ref 与 8 种查询能力，不重复内联 123 个 sample refs。path / Raw trace / CSV / protobuf payload sentinel 均为 0。unknown、同名不同 hash、缺少 `static_clicking` metric-family 授权、pending、retired 与 unlisted profile 继续 fail-closed 为 outcome-only。
- **一次 KovaaK 进程重启未破坏后续采集，但原因未证实。** `Run 1034` 后 KovaaK 在菜单阶段退出并以新 PID/HWND 启动；本次没有新的 Windows Application Error 或 CrashContext 可供归因，不能声称由 capture/encode 导致。Capture Coordinator 检测到新窗口并建立新 session，随后 `Run 1036` 的 Raw/MP4 与 finalization 全部通过；本行不替代真实 worker restart Gate。
- **Task 4 集成 Gate 的自动化阻塞与 Coach 对话死循环已收敛，但完整 knowledge/plan field chain 仍保持诚实边界。** 串行 warm-cache field replay 使用正式 API 新建 `Analysis 22 / 23 / 24 / 25`：Static `14.097s / 13.250 CPU-s / 189.1 MiB peak WS / 123 rows`，Dynamic `85.938s / 394.516 CPU-s / 373.1 MiB / 106 rows`，Tracking `127.611s / 533.703 CPU-s / 393.2 MiB / 382 aggregate rows`，Switching `75.123s / 299.172 CPU-s / 380.2 MiB / 83 rows`；均 1 次 attempt、公开 DTO privacy sentinel 通过。四条 Coach v3 投影约 `30.1 / 10.6 / 22.8 / 9.7 KB`、各有 8 种查询能力。2026-07-29 的四份 field DB 副本复用了已保存的 ready DeepSeek Provider：Static `Analysis 22` 第一轮从 `intake` 推进到 `hypothesize / version 2`，第二轮回复不再重复原 discriminator 且不再次推进；两轮均无 tool/confirmation。Dynamic `23`、Tracking `24`、Switching `25` 均返回有限 no-lesson 回复，保持 `intake / version 0`，无问句、候选、处方、tool 或 confirmation。没有 active Training Plan 或 grounded issue 的 family 仍不会伪造 knowledge/plan ref，因此不能把本轮写成每个 family 均完成 plan 写入。两条 stale/racy 测试已只在测试层收敛；完整 Coach Node 为 `156 passed`，`test_task6_backend_contracts.py` 为 `18 passed`，完整隔离 Python 为 `1440 passed, 5 skipped`，compileall 与全仓 `git diff --check` 通过。
- **发布状态仍为 No-Go。** Static、Dynamic 与 Switching exact scenario 已 production-active 并完成当前 NVIDIA 单机 field verification；Dynamic 当前只开放 accepted-quality 的 click-relative geometry，低覆盖趋势、target outcome、identity/re-entry 与无依据 Coach knowledge candidate 继续 fail-closed。Switching 只开放 Stats-bounded、event-local transition/acquire/settle/path，不声明 selection、first-shot、first-damage、persistent identity 或 re-entry。真实 DeepSeek 基础对话已在隔离 field DB 副本通过，但 OAuth/device-code、长对话/中断恢复仍未替代发布 Provider Gate；高 polling-rate、AMD/Intel、真实 worker 进程重启恢复与发布工程也仍是 release Gate。`<=130s` Tracking latency 目标和高频 status transport 残余继续保留。

### 2026-07-26 Task 12 integrated field Gate

- **真实 Tracking 证据源通过当前 NVIDIA 单机行。** `Run 479` 为 `WHJ SmoothStrafeSphere Easy` 普通 60 秒 Challenge，Stats / Performance / Raw / MP4 全部 available；Raw 为 `51,443` points、`0` drop，MP4 为 `3,632` packets、约 `60.8 MB`、`0` encoder/drop error。本行不替代高 polling-rate、AMD/Intel 或其它 launch family Gate。
- **同一 Run 的 input-native 与 multimodal 已形成真实产品链证据。** `Analysis 1` 保持 input-native / outcome-only 诚实降级；修复后的 `Analysis 3` 在 `149s` 完成 `continuous_tracking.v1`，quality `accepted`、target coverage `99.944%`、crosshair coverage `100%`，生成 15 个 metric records 与 3 个 EvidenceSegments，规范化 artifact 与独立基准一致。focused Python 为 `97 passed`，完整 Python 为 `1288 passed, 5 skipped`，compileall 与 `git diff --check` 通过。
- **`Analysis 3` 的 Desktop Analysis / evidence / Coach-safe / profile 链已回放。** Tauri managed MP4 为 path-free `aiming-cookie-media` URL，`readyState=4`、时长 60 秒并真实播放；3 个 EvidenceSegments 分别 seek 到 `33ms` / `55,766ms` / `50ms`。`coach_turn_context.v1` 携带 `coach_diagnostic_context.v3`，evidence tool 返回全部 3 个 segment，`profile_contribution.v1` 写入 5 个 exact-scenario dimensions；path / Raw trace / CSV / protobuf / private payload / secret / token / traceback sentinel 未进入 API、context、tool result 或前端文本。Provider 未配置时 Coach 仅显示激活入口，没有对话、context 或 credential 输入。
- **video-fallback 后端语义正确，但前端回放链未通过。** 同一 `Run 479` 的 `Analysis 4` 在 1 秒内完成，只声明 Stats + MP4、0 个 Tracking 机制指标、无 Raw provenance，并 fail-closed 为 `scenario_outcome_only`。managed media 本身可加载 60 秒 MP4 并真实播放，但 `/api/sessions/4/evidence-segments` 返回普通 404，当前 `VideoView` 因此误显示“视觉证据当前不可用”。该 integration contract 为 release blocker。
- **真实数据页暴露了独立 P1 前端交付缺口。** 共享 analyzer limitations 被逐指标重复渲染；CV artifact 已含 loss/reacquisition 与 ProcessedEventTable，页面却因未接线而显示“事件分布不可用”；轨迹缺少安全降采样 read model，跨记录趋势也被固定为 Analysis 空占位。这些不是 CV 无数据，不得用更多占位文案伪装完成。
- **retry / restart / deletion / owner / idempotency 主链通过，Tasks 读模型有待修一致性。** `Analysis 2` 失败 attempt 与 `Analysis 3` 成功 attempt 在重启后完整找回；重放同一 idempotency key 仍返回 `Analysis 4`。隔离删除探针 `Analysis 5` 后 session 不可用、Coach ref 为 deleted，Run-owned MP4 仍返回 `206`。非 owner 读 Analysis / video 返回 `403`，attach context 返回 `404`。但同一 `stale_lease_exhausted` 在 Session 中是 `local_cv_runtime`、Tasks 中却是 `network`，旧 attempt 的 `finished_at` 为异常 `2099-01-01`，需独立修复。
- **正常关闭完成但 capture 日志观察重新升级。** 本次关闭没有新增 `capture_export_failed`，`capture_control_unavailable` 与 ASGI traceback 均为 0，Tauri / runtime / CDP / Next / proxy 进程与端口全部清理。但长运行期间在 capture disabled 与 KovaaK absent 时仍记录 3 条 `capture_export_failed`，应作为 release 回归项，不能再仅依赖最小启停 smoke 声称已收敛。

### 2026-07-26 Frontend reconstruction closeout snapshot

- **Frontend reconstruction Task 2–7 已完成当前实现与计划内自动化验证，但不代表 release-ready。** Task 2 落地 executable tokens、light/dark/system theme 与最小 primitives；Task 3–6 完成条件启动、Provider onboarding、App shell、New Analysis、Tasks、History、Analysis workspace、Coach sidebar 与 Settings。Task 7 初始 Browser E2E 与 managed media 回归已闭合；发布前交互修复后，最新完整 Browser suite 为 `43 passed, 3 environment-conditional skipped`，截图基线为 `14` 张，accessibility 为 `5/5`，真实 Tauri WebView Coach 三形态 focused smoke 为 `1 passed`。Desktop matrix 已覆盖 sidecar READY、共享 IA、采集 gate、文件选择、Raw Input/回放缓冲状态、窗口尺寸、Storage read model 与 WebView reload 后任务找回；后者不是“真实 worker 进程重启恢复”的替代证据。
- **Managed Run-owned MP4 传输与带 EvidenceSegment 的回放路径已通过，但 video-fallback 集成未闭合。** Browser 通过 owner-scoped range streaming 播放，Tauri 通过 path-free `aiming-cookie-media` managed protocol 播放同一 Run-owned MP4；真实 managed MP4 可加载并返回 `206`，来源被移除时返回 `managed_video_unavailable.v1` / HTTP `410`。EvidenceSegment 继续只暴露相对 canonical window 的 seek anchor，不向 WebView、Coach 或 DTO 暴露绝对路径。Task 12 实机发现无 derived EvidenceSegment 的 video-fallback 仍被 `VideoView` 绑定到 evidence-segments 接口，该页面回放继续是 blocker。
...
- **发布前 UI 交互修复已收口。** Coach 父网格与侧栏共享宽度 state，分隔条使用 pointer capture 并保留键盘步进；History overlays 统一使用 Dialog/Drawer primitives，Tabs 使用 roving focus，Toast 有独立关闭按钮；按钮、Coach overlay 与 onboarding 只使用已批准的克制动效，并在 reduced motion 下即时化。该批次已提交为 `2b75dfd`。
- **Shutdown ordering race 已修复并纳入回归，但长运行日志仍未收敛。** Tauri 在 `CloseRequested` 即发起幂等 shutdown，Python 先停止 capture-control 消费方再关闭原生 endpoint/服务；既有真实 `Alt+F4` targeted regression 中 `capture_control_unavailable`、`capture_export_failed`、ASGI traceback 均为 `0`。后续独立最小启停仍干净，但两次长 UI/Task 12 smoke 分别出现 2 条与 3 条 `capture_export_failed`；Task 12 正常关闭未新增日志并干净退出。发布回归必须同时覆盖长运行和关闭阶段，不得只看最小启停。
- **发布状态仍为 No-Go。** 真实 Tracking Run 已覆盖 input-native、multimodal 与 video-fallback 后端语义，但 video-fallback 正式页面回放仍断链，Analysis Data 也有 P1 信息架构/数据接线缺口。Static、Dynamic、Switching 仍缺各自 production-active exact scenario 实证，并且仍需高 polling-rate correctness/性能、AMD/Intel 硬件编码与回放、真实 Provider 网络和 OAuth/device-code、真实 worker 进程重启恢复，以及 installer、版本、签名、公证、updater、校验值、真实下载链接和发布验证。Browser fixture、单一 NVIDIA 结果或当前 Tauri smoke 均不能替代这些 release Gate。

### 2026-07-24 Backend closeout snapshot

- **Git 边界：本节的 Coach/family/tracking closeout 与 full-review remediation 已从审计时的 dirty worktree 拆为本地 `main` 的分析、Coach、platform 三个代码 commit；本文随第四个文档批次提交。** 当前没有 push，不能用本节描述声称远端或其它 checkout 已具备这些能力。
- Full review 的当前正确性修复已在本地代码批次收口：测试 DB 隔离、SQLite transaction ownership、capture process-exit release、Raw completeness receipt、visual/native quality、stale upload recovery 与 finalizer serialization 已通过主审；随后只实施了 Knowledge validator/secret/cwd 小漂移、一次重复 trajectory derive 删除，以及 snapshot/error/EvidenceSegment 的现有 leaf contract。Wave 2 主审为 Python `414 passed, 1 skipped`、MSVC Raw `24 passed, 1 ignored`；Wave 3 主审为 Python `406 passed`、Coach `72 passed`。最终安全全量矩阵为 Python `1230 passed, 5 skipped`、Coach `72 passed`、frontend `4 passed`、MSVC Rust `70 passed, 7 ignored`，Rust format/check 与全仓 `git diff --check` 通过。严格 family lock、完整嵌套 public result DTO、Storage/History 长期规模、Coach sidecar ownership、frontend adapters、CI/toolchain 与实机/发布矩阵继续是明确 No-Go/Gate，不以新增代码伪装完成。
- 已复用旧的 `kovaak_tracker/tracking.py` 语义：精确 `b2ae4a24b710e36afc6e57c61f590ab4`、`1920x1080`、单目标场景使用黑球 HSV 初始化 + CSRT 连续跟踪；不回退 KCF，不重新把旧 CSV/preview 管线接进生产。真实 60 秒 Run 52207 只读回放为 `3599/3600` 有目标样本、`99.972%` coverage、单一 track、Tracking quality accepted。
- 真正的 `analysis.create_from_run -> Worker -> evidence -> profile` 隔离回放已通过：`analysis:6` 为 `continuous_tracking.v1`，生成 5 个 available deterministic metrics、4 张 ProcessedEventTable、3 个 EvidenceSegment，managed MP4 可 seek，画像形成 5 个 exact-scenario dimensions；Coach 默认 context 为 v3，可查询 evidence 并读取 profile snapshot。
- Task 11 已完成后端闭环：partial Analysis 只按单项 metric 准入画像；删除 reconciliation 使用 tombstone owner 并在画像失效失败时保留 tombstone；训练计划条目、用户 execution、matched/near-transfer retest 有 owner-scoped、idempotent HTTP/command 合同。新增 `frontend_evidence_segments.v1` / `evidence_segment_playback.v1` 只提供安全 metadata 与相对 seek anchor，不返回视频/帧/路径。
- Task 12 后端验收为完整 Python `1202 passed, 5 skipped`、Coach TypeScript `71 passed`、compileall 与 `git diff --check` 通过。Windows 测试夹具已修正为先关闭 SQLite 连接再删除测试库，消除长测试轮次中的随机文件锁；未改产品数据库生命周期。
- 本次没有创建前端文件。正式前端仍按 active frontend reconstruction plan 等待点点指定具体 Task；后端当前可交接，但整体发布仍受已有 No-Go Gate 和尚未激活的其它 family scenario manifest 约束，不能把 tracking 单场景回放冒充 static/dynamic/switching 全部 production-ready。
...
- 当前发布状态仍为 **No-Go**。
- Versioned Coach Knowledge Registry v1 的 43 条 entry 仅是 legacy/historical contract，不是当前 Registry 数量。当前默认 `2026-07-29.v4` 为 19 entries / 27 sources：10 个迁移条目因 capability/字段语义变化从 `@1` 升至 `@2`，另有 9 篇 Raw Input 作者级低权限 `@1` 条目。v1-v3 按显式版本继续精确可读；新 issue 以 stable observation + exact Registry refs 闭合到 Coach，历史无 refs issue 仅可展示且不能写 Training Plan。Task 3 受影响验证为 Python `358 passed`、Coach runtime `165 passed`、frontend contract/source `22 passed`；前端 `tsconfig` 现精确排除 Git 已忽略的 `src-tauri/target` 二进制构建产物，完整 `type-check` 已恢复通过。
- **2026-07-29 Analysis / Coach knowledge remediation Task 4 已完成。** 前端只在 `observation_ref + knowledge_registry_version + knowledge_entry_refs` 均合法时把新 issue 呈现为 Registry-backed：显示“重点观察”、证据等级、候选解释与规则化练习建议；没有完整 refs 的新/旧结果不再被误标为新知识，旧 `root_causes` / `prescriptions` 仍可读但统一降级为“历史候选说明”，physical 层明确是动作机制候选。Analysis 没有复制完整处方，Provider Coach 的自然表达、API persistence、导航和交互均未改。验证为 frontend contract/source `11 passed`、`npm.cmd run type-check`、`npm.cmd run build`、生产服务下 Browser smoke + accessibility `16 passed`，以及人工检查后 `1280x820` / `960x640` 的两张截图回归通过；本地 Edge 测试仅临时绕过系统代理，未修改系统代理或产品运行时配置。
...
- 2026-07-21 Complete Coach Task 1-5A 已在当前未提交 worktree 实现：canonical time、fail-closed ScenarioProfile、统一 Signal/Event/Metric/Evidence artifact、bounded evidence broker、static clicking 迁移，以及覆盖每次 static flick 的 ProcessedEventTable 均已接通。新 Analysis 使用 `coach_diagnostic_context.v3` 提供完整字段目录/行数/完整度，Coach 可通过固定 exact/rank/filter/aggregate/co-occurrence/sequence/compare 查询全部 processed rows；Raw、MP4、frame、路径和未注册 payload 仍不可见，deterministic diagnosis 只是可被证据与反例挑战的候选观察。
- Task 5A focused Python 为 `298 passed`，Coach runtime 全量为 `69 passed`。2026-07-23 Task 6 detector v2 已完成 dynamic-only candidate Gate：producer `visual_round_detector.circularity_0_60_center_overlay_0_50.v2` 可受限恢复中心单目标准星遮挡；多目标粘连明确输出 ambiguity，不猜 target identity。独立 calibration 为 `60/62`、holdout 为 `74/78`，合计 `134/140` matched targets、`0` observed false positives、center P95 `2.96 px`、radius error `0.76 px`、coverage `95.71%`；结果仅绑定 exact scenario/config，不支持 tracking/switching 或跨 Run 泛化。真实 MP4 opt-in replay 已通过，`41/41` 条已复核 stable click-center 几何保持不变；identity continuity/re-entry 未观测，production registry 继续为空。
- 2026-07-22 Task 10R cross-family Coach knowledge research 已完成：已把 Voltaic/Raw Input/真人教练的 reading、speed matching、confirmation、smoothness、tension、dynamic clicking、tracking 与 switching 教学词映射到当前可观测 facts、替代解释、禁止推断、cue、剂量边界和 matched/near-transfer retest。Task 10 Registry v2、migration audit、Python/TypeScript retrieval 与新 turn 默认 v2 已在当前未提交 worktree 实现并通过自动化；v1 historical resolution 保持精确可读。Task 6A 的版本化 validated OutcomeAssociation 合同与自动化已经闭合；真实 association rule/profile 仍需稳定 identity 独立复核，因此 Task 10 仍不能标为跨-family production-ready，但不得再重复实现 v2。
- 2026-07-23 Complete Coach 四态审计：Task 6、7、8、9 的代码与 synthetic/automated Gate 已存在；detector v2 与 reviewed association ledger 可严格形成 `36` 条 `validated_aligned` candidate association，其余 click 继续 unavailable。实际 `video preprocessing -> track -> association -> dynamic analyzer` 的 candidate-mode 联合回放与同条件 dynamic baseline 尚未闭合，所以不能把 reviewed ledger 结果直接当成生产 E2E。Task 8/9 仍缺同 family 完整真实 Run、identity continuity 与 occlusion/re-entry 标注。`_REVIEWED_VISUAL_PRODUCERS`、Scenario Registry、outcome rule registry 与 launch manifest 均为空，因此真实新路径继续 fail closed 为 outcome-only/partial；没有任何 dynamic/tracking/switching family 生产启用。Task 11 未开始并等待这些上游 Gate。
- 2026-07-23 Task 6A validated outcome association repair 已完成代码、自动化与 reviewed candidate replay：timescale field audit 的 `55` 个 one-shot kill 均有唯一 `0..50 ms` Raw click 候选、真实延迟 `0..8 ms`；`44` 个唯一准星中心重叠候选经独立复核为 `41 stable / 3 not_stable`，其中现有 `1 px` inner-hitbox Gate 可严格形成 `36` 条 `validated_aligned` association，其余样本继续 unavailable。detector v2 已恢复 `029/043` 的唯一中心目标 observation，但人工 truth 仍为 `not_stable`，不会自动升级；`033` 明确返回 merged ambiguity，不猜 identity。opt-in association replay 为 `11 passed`，完整 MP4 opt-in replay为 `1 passed`，完整 Python 为 `1163 passed, 5 skipped`；真实 replay 绑定本地 exact source hashes，所有 production registry/manifest 继续为空。
...
- 旧 prototype 已删除，正式 Provider onboarding、App shell、New Analysis、Tasks、History、Analysis workspace、Coach sidebar 与 Settings 已按 reconstruction plan 重建，并具备 Browser/Tauri 用户路径；这只表示 user-reachable，不表示 release-ready。
- Capture Coordinator、Run Finalizer、pending Run readiness、Run-owned MP4 和单证据 Storage 手动删除 capability 已通过自动化与当前 NVIDIA 实机行；AMD/Intel 保持未验证且 v1 不承诺支持，不作为当前发布 No-Go。GPU-resident Media Foundation H.264、300 秒 encoded replay ring 与无整窗重编码 MP4 mux 已通过真实 normal Challenge、timescale-extended 与 Restart canonical export；short-pause 仍在缺少稳定毫秒级 wall duration source 时 fail closed。
- Coach sidebar 与 0..N context 能力已 user-reachable；已保存 Provider 的连接与真实 Coach 回合通过当前隔离验证，长对话继续/停止/失败轮次恢复已有实现和回归。OAuth/device-code 仍延后，不阻塞当前范围。
- 真实 Tracking `Run 479` 已形成 input-native、multimodal 与 video-fallback Desktop E2E；正式视频页已与 EvidenceSegment presence 解耦，Data 页已接入 bounded event/error projection。Static、Dynamic、Tracking、Switching 均已有各自 production-active exact scenario 实证。
- Windows 高 polling-rate 鼠标 correctness/性能与完整发布矩阵尚未通过；
...
1. Prototype 已删除且不得恢复；Task 2–7 已完成 executable tokens/theme/primitives、正式路由、App shell、Provider onboarding、New Analysis、Tasks、History、Analysis workspace、Coach sidebar、Settings 与 Browser/Desktop E2E。
2. Browser 与 Desktop 共用 IA、主题、组件和状态语义；Desktop-only 的 Raw Input、文件选择、managed media 与 capture gate 通过 capability adapter 暴露，不由 Browser fixture 伪装。
3. Task 12 remediation 已接入 bounded Analysis Data projection，修复无 EvidenceSegment 的 video-fallback 播放，并保持 derived artifact 损坏 fail-closed；当前正式截图基线为 14 张，Browser Playwright 为 `47 passed, 3 Desktop-conditional skipped`。
4. 有限 KovaaK 成绩同步后端、去身份 Coach 摘要和公开读取合同已完成；Onboarding / Settings 输入与最小成绩视图仍等待独立 OpenDesign / 前端 Task，不建立独立 Benchmark 页面。
5. 当前前端剩余工作属于发布 Gate 与真实环境验证：真实 Tauri product-path、高 polling-rate、Tracking 时延目标和发布工程；不以继续堆页面或 fixture 替代。
...
4. Coach 已能通过与 UI route 共用的 owner-scoped handler 查询 Run / History / Analysis、比较分析、生成 typed navigation/evidence/video-time event、创建或重试 Analysis，并生成、保存、激活、暂停、调整和 review 带版本理由与 verification targets 的 Training Plan；正式 Coach sidebar 已消费 0..N context、agent-run 状态、停止/重试与确认流。普通明确写操作直接执行，Coach 推断写操作和替换 active plan 走持久确认，命令具有 append-only audit 与独立 idempotency replay；
5. API key/OAuth credential 明文本地 persistence 已按 local-first 决策完成；进行中的 auth operation 不持久化 provider-specific 中间状态，backend/sidecar 重启后明确 interrupted 并允许重试；正式 Provider Settings 已提供 credential 只换/删不回显与连接测试，已保存 Provider 的真实连接/Coach 回合通过；OAuth/device-code 后续再接，不阻塞当前范围。
6. Versioned Knowledge Registry 是唯一运行时知识正文；历史 v1 的 43 条 entry 与当前 v3/v4 资产不得混算。当前默认 `2026-07-29.v4` 为 19 entries / 27 sources，已新增严格 capability 前缀与作者级社区条目；trace 继续只保存 registry/entry/version/source/claim refs，不保存知识正文、路径、secret 或 raw payload。
7. 点点提出后续 Coach 讲解需要更直观：先说人话，再按需展开指标，并探索图标、轨迹/阶段动画、问题区段标记和前后对比；这只是待讨论的产品/视觉方向，尚未冻结进 frontend contract，也未实施。

## 4. 下一步

> **新 session 接力点：** Task 12 field blocker remediation 已完成，但完整 Task 12 release Gate 仍 active。当前 dirty worktree 尚未提交、未推送；后续应先按授权边界复核并分批提交，不重新采集或扩大产品合同。

1. 保持 Static、Dynamic、Tracking、Switching 四个 exact family 的当前 field evidence 与 fail-closed 边界；不把单机 NVIDIA 证据扩大成 AMD/Intel 支持承诺。
2. Tracking production 路径当前 exact-parity 中位数为 `148.039s`，未达到 `<=130s`。半分辨率 + ROI candidate 只有 reference-relative 结果，在缺少 annotation quality Gate 时不得激活。
3. 高 polling-rate correctness/性能、真实 Tauri product-path、Tracking 时延目标与 installer/签名/updater/下载验证继续保持 release Gate；OAuth/device-code 与 AMD/Intel 不阻塞当前范围。
4. 任何新的真实采集仍必须重新执行 capture-ready → 点点明确“开始” → capture-started 握手；不得静默读取或修改产品数据库。
...
| Task 6 History / evidence replay | **backend + frontend completed（当前平台）** | backend light list/lazy detail、frozen availability/trace、evidence locator、共享 replay Gate 与严格 comparability 已验收；正式 History / Run inspector 已接入并通过 Browser E2E，真实跨记录 comparability 仍需 field Gate |
| Task 7 Benchmark local domain | local store / API、有限 KovaaK 同步、去身份 Coach 摘要已实现；历史 prototype UI 已由 Frontend Task 1 删除 | Onboarding / Settings 输入和最小成绩视图仍待独立 OpenDesign / 前端 Task；不建立独立 Benchmark 页面 |

Frontend reconstruction plan 的 Task 1–7 已逐项授权并完成，计划已移入 completed archive；prototype inventory 与 adapter boundary 保留，正式 tokens/theme/primitives、产品路由、History、Analysis workspace、Coach、Settings、Browser/Desktop E2E、截图与 accessibility 均已实现。真实硬件、Provider 与发布 Gate 继续由 Roadmap/Task 12 管理。

Coach productization 的 explanation、input-native core metrics、完整 Pi catalog/custom profile/selected turn、local credential commands、knowledge/product commands 与正式 Coach sidebar/Settings 已实现；已保存 Provider 的真实连接/Coach 回合与长对话继续/停止/失败轮次恢复已有验证，OAuth/device-code 后续再接。
...
- [`archive/history/PROGRESS-2026-07-12-desktop-slice.md`](PROGRESS-2026-07-12-desktop-slice.md)

2026-06-27 至 2026-07-10 的历史流水保存在：

- [`archive/history/PROGRESS-2026-06-27-to-2026-07-10.md`](PROGRESS-2026-06-27-to-2026-07-10.md)
