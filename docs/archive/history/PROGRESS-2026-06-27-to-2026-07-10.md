# Aiming Cookie — 历史研发记录（2026-06-27 至 2026-07-10）

> **状态：历史归档。** 本文只用于追溯旧决策、旧实现和遗留问题，不是当前产品、架构或施工事实源。
> 当前状态请看 `docs/PROGRESS.md`；产品与施工以 `docs/PRD.md`、`docs/ARCHITECTURE.md`、`docs/ROADMAP.md` 和活跃 implementation plan 为准。
...
4. **商业 D→B 分阶段**：D 邀请制内测 → B freemium（分阶段开放深度能力）
...
2. **核心流程决策**（brainstorm 过程，逐一确认）：
   - 诊断与教练的能力切分（教练侧走 LLM）—— 切分点干净，代码原生支持
...
3. **形态决策演进**：
   ...
   - 境外部署 + Cloudflare（暂不涉及国内备案，不迁国内）
4. **云端方案 A**：境外小 VPS（FastAPI 鉴权 + LLM 代理 + 计费）+ Cloudflare Pages（landing）+ GitHub Releases（分发）。CV 本地后云端只剩轻量级。

### 文档清理 + 对齐
- **对齐 PRD 冲突**：IA spec（§2.2 / §4.1 / §5.1 邀请码 → 开放注册）、webapp spec（备案段加 PRD 演进头注）、product-strategy（备案口径对齐）、PRD §10 残留"邀请制"修正
...
- ⚠️ **subagent model 路由**：供应商侧当日发生模型档位变更，是本次 review 集体 hallucination 的根因。点点后台发现，自行 fix 路由。教训：subagent 产出必须读代码核实。
