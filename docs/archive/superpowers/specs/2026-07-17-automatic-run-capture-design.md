...
当前事实前提是：Aiming Cookie 可以检测 KovaaK 进程并监视 Stats / Performance 文件，但没有可靠的实时 Challenge start/end 事件；倒计时可由用户关闭，也不能成为正确性来源。Stats / Performance 在 Challenge 结束后到达并提供构造 canonical wall window 所需的 source records；Stats 的 time-of-day 单独只是 observation，不能伪造 UTC 或独自形成 `CanonicalTimeWindow`。因此自动采集必须采用“进程 gate 内连续 Raw + 硬件编码回放缓冲，文件到达后事后切窗”，不能把进程存在或视觉提示误写成 Challenge 正在进行。
...
自动采集与后处理统一使用 `time_alignment.v2`：Stats `Challenge Start` 提供 Challenge-relative 毫秒锚点，Performance 提供秒级日期身份；只有可信 mapping 同时成立时，才形成供 Raw/MP4 对齐的 `CanonicalTimeWindow`。Stats time-of-day 单独存在时不得写成 UTC；此时仍可保留 source observation 与 partial/unavailable 诊断，不能声明跨来源 canonical aligned。窗口为 `[start_ms, end_ms)`，计时窗口使用 `time_limit / timescale + pause_duration`，可信终止事件优先。Raw ACRI v1 字节格式保持兼容；QPC/UTC 相关元数据仅作为捕获边界 provenance，不能替代产品时间轴。
...
Run Finalizer 使用稳定的 Stats / Performance source identity 与经可信 mapping 形成的 canonical Challenge window，把连续采集流切成独立 Run：
...
- Stats time-of-day 不能单独补齐或重写 canonical window；缺少可信 mapping 时保留 source observation 与 partial/unavailable evidence，不把 Raw/MP4 标为 canonical aligned；
