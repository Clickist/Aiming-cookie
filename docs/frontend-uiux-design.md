...
> **2026-08-10 IA 同步：** Coach 为默认主工作区；`/` 为 Coach 首页，`/s?sessionId=<id>` 为指定会话；左侧 Session rail 为唯一主导航，用户可见一级消费面只有 Coach、History 与 Settings。Tasks 与独立 Analysis 页面退役，旧 URL 只跳转 History；Analysis 数据对象和可复用数据/视频组件继续作为内部能力保留。桌面最小内容宽度为 `1180px`，不维护更窄窗口的第二套布局；History 消费内容最大宽度为 `580px`，Settings 为 `1040px`，均居中。旧侧栏和独立报告页文字仅保留为决策演进记录，不得据此恢复页面。
>
...
Aiming Cookie 的主要形态是：**左侧 Session rail（顶部为 Logo 品牌行）+ 右侧主工作区**。Coach 会话在对话区上方另有一条 44px 状态顶栏（状态点、会话标题、讨论条槽位）；空对话首页不渲染该顶栏。

```text
┌───────────────┬──────────────────────────────────────────┐
│ Logo（静态）  │ Coach 会话顶栏 44px                      │
│ 品牌行        │ ● 状态点  会话标题  讨论条槽位            │
├───────────────┼──────────────────────────────────────────┤
...
- 左侧 Session rail 是应用导航主体，不是网站导航栏；Coach 会话顶栏只是会话状态条。
- 主工作区就是 Coach，不能再把 Coach 降级为右侧可选聊天层。
- 左侧 Session rail 是唯一主导航；History、Settings、搜索和会话管理都从 rail 进入。
- Coach 会话顶栏不提供 Coach 开关、History、Tasks 或新建分析入口，只显示状态点、会话标题与讨论条槽位；空对话首页不渲染顶栏。
- 页面切换不能清空 Coach 会话；rail 只承载会话导航，不承载另一套对话数据层。
- Logo 属于 Session rail 顶部的品牌行，静态、不可点击，不承担导航；不存在横跨全宽的采集状态、分析指示或 Provider 状态顶栏。

```text
Session rail 品牌行：Logo（不可点击）｜应用名
Coach 会话顶栏 44px：状态点｜会话标题｜讨论条槽位
```

- “新建对话”只在 Session rail 中创建草稿，首条消息成功后才持久化 Session；分析不再是顶栏主操作。
- 产品没有账户菜单；Settings 从 Session rail 底部进入。当前会话与分析状态放在主工作区，不塞进导航栏。
...
History 使用同一页面承载待分析训练、其它训练记录与分析记录；主要内容在可用区域内以 `580px` 最大宽度居中。它不是独立报告页，也不把数据和图表横向撑满全屏。
...
- 聊天输入区提供当前 Provider 内的模型菜单（composer 底部、向上弹出）：切换即更新全局默认 profile 的 model_id 并持久化，Settings 同步显示；切换 Provider 仍只在 Settings 中完成。菜单在 profile 为内置 Provider 且其固定目录提供至少两个模型时渲染，自定义 Provider 只要发现至少一个模型即渲染（单模型也显示当前模型名）。当前模型声明支持推理时，另渲染独立的「思考力度」菜单（默认/关闭/极简/低/中/高），调整同样只对下一段回复生效并写回默认 profile。
...
- 左侧 Session rail 顶部为静态 Logo 品牌行；Coach 会话在对话区上方有 44px 状态顶栏，只显示状态点、会话标题与讨论条槽位，空对话首页不渲染；不存在横跨全宽的采集状态、分析指示或 Provider 状态顶栏；
...
- Session rail 固定宽度，暂无收起入口；
- 视频区在需要时打开或关闭，关闭后对话列恢复占满 rail 右侧；
- 在支持 Coach 的页面之间切换时，保留会话、草稿和上下文状态；
...
顶栏不显示 Coach 名称（2026-09-13 点点拍板：会话标题与状态点已承担该职责，不加名称元素）。
...
- 多行文本；
- 发送键随运行态变形：空闲时为发送；运行中且输入为空时为停止生成；运行中且已输入时为发送（点击或 Enter 自动入队）；
- 运行中已入队的消息以 composer 上方队列 chips 呈现，可逐条「立即打断发送」、回填编辑或取消；
- 查看本次附带的上下文；
- 移除不想发送的上下文；
...
| 页面类型 | Coach 规则 | 初始状态 |
|---|---|---|
| 首次 onboarding | 不提供空 Coach 对话；使用结构化 Provider 激活和本地 fallback | 不显示 Session rail |
| 设置（含 Provider） | 不提供 Coach 对话；Settings 自身显示 Provider/auth 状态 | 不显示 Session rail |
...
- Coach 回复和已开始的后台操作在页面切换后可以继续，完成后使用低干扰状态提醒；
...
- Session rail 固定宽度、无收起/展开状态；路由切换时保留会话、草稿、上下文和回复进度；
- 视频关闭后，对话列恢复占满 rail 右侧；
- History 消费内容使用 `580px` 最大宽度并居中，Settings 使用 `1040px` 最大宽度并居中；
