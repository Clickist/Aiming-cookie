...
诊断回答“重点观察到什么、哪些解释仍是候选、下一步如何验证”，并作为分析工作区默认入口。必须包含：

1. **重点观察**：用白话说明当前证据支持的观察，优先于指标和长篇讲解；不把它写成已证实的根因或“最需要处理”的问题；
2. **瞄准表现类型**：显示 profile 标签及必要的可信度说明，但不能把概率包装成绝对结论；
3. **观察项列表**：每项显示观察名称、严重程度、优先级理由，以及“查看证据”“查看指标”和“问 Coach”；第一版重点展开不超过三个观察项；
4. **候选解释与证据等级**：具有完整 `observation_ref + knowledge_registry_version + knowledge_entry_refs` 的新结果显示候选解释和 claim level；不把 physical limitation、动作机制或身体状态写成已证实根因；
5. **规则化练习建议**：新结果只提供单一重点和可比复测方向，不在 Analysis 中复制具体场景、cue、剂量或完整处方；这些教学正文由 versioned Knowledge Registry 和 Coach 组织。历史 `root_causes` / `prescriptions` 仍可读，但必须统一标为“历史候选说明”；
