...
3. **服务器就绪后**：官方托管服务 HTTPS（客户端仅改一行地址，前置条件是服务端配域名与证书）。

### 已拍板待排期（2026-09-14 点点决策，时点未定）

1. **发版更新公告**：每次发新版后，应用内弹「本次更新了什么」公告弹窗（按版本只弹一次、可关闭），落地页同步增加更新公告/更新日志区块；内容随版本号走单一事实源，纳入发版流程一起产出与验证。

### 当前优先级（2026-09-06 快照，1.0.0 后未复核；完成项移入 PROGRESS）

1. **内测循环**：两位内测用户复测「官方托管服务 + AC 搭配」（v0.1.14 上下文治理已换 microcompact）＋ 新版本经自动更新触达的验证；
...
4. **营销**：B站口播定稿进入拍摄（录屏基准 v0.1.14）；HyperFrames hook 工作台推进（本地独立工程）；
5. **售卖链路**（拍板为不急）：官方托管套餐 + BYOK 双轨的完整售卖流程；
...
已完成发版工程成果（OpenDesign 桌面设计与 Frontend reconstruction Task 1–7、Raw Input 1000 Hz canonical 归一化、落地页与 SEO/D1 stats Worker、真实截图与演示 MP4、installer/NSIS 打包链、自动更新三件套、v0.1.10–v0.1.15 与 v1.0.0、v1.0.1 的实际发版验证）已并入 [`PROGRESS.md`](PROGRESS.md)，本文不重复维护。发布工程剩余：
...
- [`archive/superpowers/plans/2026-07-20-complete-coach-analysis-context-v1.md`](archive/superpowers/plans/2026-07-20-complete-coach-analysis-context-v1.md)：完整 Coach 的 Run facts、processed event tables、visual producer、跨 family analyzer、Knowledge Registry v2、画像/计划/复测与 release Gate；
- [`archive/superpowers/plans/2026-07-13-reflek-capability-adoption.md`](archive/superpowers/plans/2026-07-13-reflek-capability-adoption.md)：Analysis/evidence correctness、Run/trace 与 Coach 结构化能力；
- [`archive/superpowers/plans/2026-07-13-coach-productization-provider-management.md`](archive/superpowers/plans/2026-07-13-coach-productization-provider-management.md)：解释/处方合同、input-native 核心指标、Pi provider/model/auth、用户级 Coach 命令与 Provider Settings；
...
- uploading/importing/running 崩溃后重启能恢复到可用状态；
...
- AMD/Intel 明确标为未验证且 v1 不承诺支持，不作为内测 No-Go 条件；
- 至少一条真实素材端到端路径通过；
- 至少验证单局 normal、multimodal、video fallback 和手动 fallback 四条核心路径。

### Part B — 正式发布 Gate

以下 Gate 在内测后、正式发布前需满足。
...
- 所有受保护接口验证 launch token；token 不持久化、不泄露日志、不传播给无关子进程；
...
内测阶段只需满足 Part A Gate。未满足 Part A 时不发布内测。Part B Gate 在正式发布前满足。
