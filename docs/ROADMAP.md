...
2. 将自动采集的 Raw + 仅 KovaaK 窗口有界回放缓冲与 Stats/Performance 事后 Run finalization 作为 Desktop 主训练来源；单局确认、多局选一条，其余保留待分析；手动 MP4 + Stats 作为独立 fallback；
...
7. 自动采集局部合同已由 [`superpowers/specs/2026-07-17-automatic-run-capture-design.md`](superpowers/specs/2026-07-17-automatic-run-capture-design.md) 冻结；active [`superpowers/plans/2026-07-19-automatic-run-finalization-v1.md`](superpowers/plans/2026-07-19-automatic-run-finalization-v1.md) 负责 Capture Coordinator、Run Finalizer、Run-owned evidence、pending Run readiness 与 Storage/recovery。Frontend Task 2 可独立建立 tokens/primitives，Task 3/4/6 在对应 capability 通过前不得伪造完成状态。
